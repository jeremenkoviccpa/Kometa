...
def m1_sequence(x: Candles, window: int, lookback: int, disp: float) -> dict[str, float] | None:
    ...
    if not fvg_hi > fvg_lo:  # 5d
        return None
    leg_hi = max(h[s : x_ + 2])
    b = next((i for i in range(x_ + 2, n - 1) if c[i] > leg_hi), None)  # 5e
    if b is None or min(c[x_ + 1 :]) < fvg_lo:  # 5f: no close through the gap
        return None
    e = n - 1
    if not min(lo[e - 1 :]) <= fvg_hi:  # 5f: the pullback reaches the gap on this bar or the last
        return None
    pin = bool(
...
class SmcSniper(Strategy):
    ...
    def _structure(self, ctx: StrategyContext, sym: str) -> None:
        ...
        if min(len(d1), len(h4)) < 30 or len(h1) < 60:
            return
        bias = float(trend_state(d1.bid_h, d1.bid_l)[-1])
        if bias == 0 or float(trend_state(h4.bid_h, h4.bid_l)[-1]) != bias:
            return  # 1: no agreed bias
        sell = bias < 0
    ...
    def _refine(self, ctx: StrategyContext, sym: str) -> None:
        """Rule 3, M15 part: the newest M15 block inside the H1 zone narrows the POI."""
        ...
        fine = [
            ...
            if z[0] <= z_hi and z[1] >= z_lo and z[1] <= htf["eq"]
        ]
        if fine:
    ...
    def _hunt(self, ctx: StrategyContext, sym: str) -> list[Request]:
        ...
        if not until or not c or ctx.my_positions(sym) or ctx.my_pending(sym):
            return []
        p = ctx.params
        window, lookback = int(p["window_m"]), int(p["liq_lookback"])
        m1 = ctx.market.bars(sym, Timeframe.M1, window + lookback + 20)
        if not len(m1) or int(m1.close_time[-1]) > int(until):
            return []
        sell = bool(c["sell"])
        seq = m1_sequence(_candles(m1, sell), window, lookback, float(p["disp_atr"]))
        if seq is None:
            return []
        z_lo, z_hi = c["poi"]
        ...
        return [
            ctx.signal(
                ...
                reason=(
                    ...
                    f"sweep {sign * seq['sweep']:.2f}, CHOCH {sign * seq['choch']:.2f}, BOS, FVG retest, "
