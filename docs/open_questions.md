...
| 37 | Where the owner's assistant-written strategies run (ai/builder.py): spec 14.3 sandboxes agent-generated code in a network-less container | They are owner strategies, saved only by the owner who is responsible for them: checked in a child process with a time limit plus the AST static checks, then run in the demo process. A container sandbox comes with L1 discovery agents (slice 9.7), which generate code without the owner. |
